...
### Added
- **Merge Mode (Suika Game)**: When two speakis of similar size collide, they merge into a larger one
  - Disabled by default, toggle in Settings → スピキ → Merge (Suika) → Enabled
  - Size grows by √2 (area doubles) on each merge
  - Max size limit prevents infinite growth
  - Sound effect plays when merging
  - Configurable settings: tolerance, growth factor, max size, pop force
